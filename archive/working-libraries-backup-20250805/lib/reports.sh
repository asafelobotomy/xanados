...
generate_report() {
    ...
    if [[ -z "$report_type" || -z "$data_file" ]]; then
        log_error "Usage: generate_report <report_type> <data_file> [formats] [output_dir]"
        return 1
    fi
    ...
    if [[ -z "$output_dir" ]]; then
        ...
        if [[ $? -ne 0 ]]; then
            log_error "Failed to get default output directory"
            return 1
        fi
    fi
    ...
    log_info "Generating $report_type report from: $data_file"
    ...
    for format in "${format_array[@]}"; do
        ...
        case "$format" in
            ...
            "$REPORT_FORMAT_MARKDOWN")
                ...
                ;;
            *)
                log_warning "Unknown report format: $format"
                ;;
        esac
    done
    
    log_info "Report generation complete: $success_count/$total_count formats successful"
...
_archive_reports() {
    ...
    log_info "Checking for old reports to archive (keeping $max_reports recent reports)"
    ...
    if [[ $total_reports -gt $max_reports ]]; then
        ...
        log_info "Found $total_reports reports, archiving oldest $reports_to_archive"
        ...
        for ((i=0; i<reports_to_archive; i++)); do
            ...
            if mv "$file_to_archive" "$archive_dir/"; then
                log_info "Archived: $(basename "$file_to_archive")"
            else
                log_warning "Failed to archive: $file_to_archive"
            fi
        done
    else
        log_info "Report count ($total_reports) within limits, no archiving needed"
    fi
}
