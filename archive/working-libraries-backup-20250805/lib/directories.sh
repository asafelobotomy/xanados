...
ensure_results_structure() {
    ...
    log_debug "Creating standardized results directory structure for: $script_type"
    ...
    for subdir in "${subdirs[@]}"; do
        if ! safe_mkdir "$results_dir/$subdir"; then
            log_warning "Failed to create subdirectory: $results_dir/$subdir"
        fi
    done
...
create_standard_results_dirs() {
    ...
    log_info "Creating standardized results directory structure"
    ...
    for result_type in "${result_types[@]}"; do
        ...
        if safe_mkdir "$type_dir"; then
            log_debug "Created results directory: $type_dir"
            ...
            log_warning "Failed to create results directory: $type_dir"
        fi
    done
    ...
    log_success "Standardized results directory structure created"
}
...
clean_old_results() {
    ...
    if [[ ! -d "$base_results" ]]; then
        log_warning "Results directory does not exist: $base_results"
        return 0
    fi
    
    log_info "Cleaning results older than $days_to_keep days (type: $result_type)"
    ...
    log_success "Results cleanup completed"
}
...
get_results_filename() {
    ...
    if [[ -z "$base_name" ]]; then
        log_error "get_results_filename: base_name is required"
        return 1
    fi
...
get_log_filename() {
    ...
    if [[ -z "$script_name" ]]; then
        log_error "get_log_filename: script_name is required" 
        return 1
    fi
...
create_user_directories() {
    ...
    log_success "All user directories created successfully"
    return 0
}
...
create_project_directories() {
    ...
    log_success "All project directories created successfully"
    return 0
}
...
ensure_directory() {
    ...
    if [[ -n "$owner" ]] && [[ "$EUID" -eq 0 ]]; then
        if ! chown "$owner" "$dir" 2>/dev/null; then
            log_warning "Failed to set ownership of $dir to $owner"
        fi
    fi
...
is_safe_directory() {
    ...
    for protected_dir in "${protected[@]}"; do
        if [[ "$abs_dir" == "$protected_dir" ]]; then
            log_error "Cannot operate on protected directory: $abs_dir"
            return 1
        fi
    done
...
create_dir_with_gitkeep() {
    ...
    if ensure_directory "$dir"; then
        touch "$dir/.gitkeep"
        log_debug "Created directory with .gitkeep: $dir"
        return 0
    fi
...
list_directory_contents() {
    ...
    ls $ls_opts "$dir" 2>/dev/null || log_error "Failed to list directory contents"
}
